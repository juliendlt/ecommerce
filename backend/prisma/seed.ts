...
async function main() {
    ...
    const veloursCote = await prisma.optionValue.create({
        data: {
            label: "Velours côtelé terracotta",
            type: "tissu",
            priceOffSet: 2.5,
            isAvailable: true,
        },
    ...
    const pochette = await prisma.product.create({
        data: {
            ...
            images: {
                create: [
                    {
                        url: "https://placehold.co/600x400?text=Pochette+1",
                        alt: "Pochette zippée Liberty",
                        position: 0,
                    },
                    {
                        url: "https://placehold.co/600x400?text=Pochette+2",
                        alt: "Pochette zippée Wax",
                        position: 1,
                    },
                ],
            },
    ...
    await prisma.productOptionGroup.create({
        data: {
            ...
            position: 0,
            values: {
                connect: [{ id: libertyFleurs.id }, { id: waxBleu.id }, { id: linNaturel.id }],
            },
        },
    });
    ...
    const tablier = await prisma.product.create({
        data: {
            ...
            images: {
                create: [
                    {
                        url: "https://placehold.co/600x400?text=Tablier+1",
                        alt: "Tablier coton rayé",
                        position: 0,
                    },
                ],
            },
    ...
    await prisma.productOptionGroup.create({
        data: {
            ...
            position: 0,
            values: {
                connect: [{ id: cotonRaye.id }, { id: libertyFleurs.id }, { id: linNaturel.id }],
            },
        },
    });
    ...
    const toteBag = await prisma.product.create({
        data: {
            ...
            images: {
                create: [
                    {
                        url: "https://placehold.co/600x400?text=Tote+1",
                        alt: "Tote bag imprimé léopard",
                        position: 0,
                    },
                ],
            },
    ...
    await prisma.productOptionGroup.create({
        data: {
            ...
            position: 0,
            values: {
                connect: [{ id: imprimeLeopard.id }, { id: waxBleu.id }, { id: veloursCote.id }],
            },
        },
    });
    ...
    const gigoteuse = await prisma.product.create({
        data: {
            ...
            images: {
                create: [
                    {
                        url: "https://placehold.co/600x400?text=Gigoteuse+1",
                        alt: "Gigoteuse velours terracotta",
                        position: 0,
                    },
                ],
            },
    ...
    await prisma.productOptionGroup.create({
        data: {
            ...
            position: 0,
            values: {
                connect: [{ id: veloursCote.id }, { id: cotonRaye.id }, { id: libertyFleurs.id }],
            },
        },
    });
